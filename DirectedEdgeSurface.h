...
class DirectedEdgeSurface
    ...
    //Student Submission
    //Routine to perform loop subdivision on surface
    DirectedEdgeSurface loopSubdivision();

    //Given a vertex, return a vector of its neighbours
    std::vector<int> getNeighbours(int vertexID);

    //Given a vertex, return its normal value
    Cartesian3 calculateNormal(int vertex, std::vector<unsigned int> faceVertices);

    //Calculating edge vertices
    Cartesian3 calculateEdgeVertex(int vertex1, int vertex2, int vertex3, int edgeNumber);

    //Updating the positions of existing vertices
    Cartesian3 updateVertexPosition(int vertex, std::vector<int> vertexNeighbours);
    }; // class DirectedEdgeSurface
